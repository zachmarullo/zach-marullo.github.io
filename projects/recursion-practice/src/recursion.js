...
var sum = function(array, allSum = 0) { 
if(array.length === 0){
  return allSum;
}

  allSum += array[0];
return sum(array.slice(1), allSum);
...
var palindrome = function(string) {
  let text = string.replace(/[\s"'.,-\/#!$%\^&*;:{}=\-_`~()\\\[\]@+|?><]/g, "").toLowerCase();  
  if(text.length === 1) {
      return true;
    }
    if (text.length === 2) {
    ...
    if (text[0] === text.slice(-1)){
      return palindrome(text.slice(1, -1));
    }
    return false;
};
...
var capitalizeFirst = function(array, allCaps = []) {
  if (array.length === 1) {
...
var minimizeZeroes = function(array, newArr = []) {
  ...
  if (array.length >= 1 && array[0] !== newArr[newArr.length -1]) {
    newArr.push(array[0])
  }
